function f_TopTenCategoriesInStock(){
	$.ajax({
		type : "POST",
		url : "chart?",
		data : "chartName=TopTenCategoriesInStock",
		dataType : "json",
		async : true,
		cache : false,
		timeout : 50000,
		success : function(data) {
			var jSonObject = data;
			var categoryChartSeries = new Array();
			for(var i = 0 ; i < jSonObject.length ; i++){
				var categoryChartSerie = new Object();
				categoryChartSerie.name = jSonObject[i].categoryName;
				categoryChartSerie.data = new Array();
				categoryChartSerie.data.push(jSonObject[i].quantity);
				//alert(categoryChartSerie.data);
				categoryChartSerie.type = "column";
				categoryChartSeries.push(categoryChartSerie);
			}
			//alert(JSON.stringify(categoryChartSeries));
			$("#topTenCategoriesInStock").kendoChart({
				theme : $(document).data("kendoSkin") || "blueopal",
				title : {
					text : "Top 10 Loại Sản Phẩm Trong Kho"
				},
				legend : {
					position : "bottom"
				},
...
function f_editUI(id) {
	...
	var date = new Date();
	var editUI = "<td class='id_col'></td><td class='id_col'></td><td><input type='text' value='' id='tenLoaiSanPhamChinhSua'/></td>"
			+"<td class='ncn_col'><input type='text' value='"+date.format("dd/mm/yyyy")+"'/></td>"
...
function f_insert() {
	...
	loaiSanPham.tenLoai = $("#tenLoai").val();
	var ngayCapNhat = new Date();
	loaiSanPham.ngayCapNhat = ngayCapNhat;
	loaiSanPham.tinhTrang = $("#tinhTrangThemMoi").val();
	//alert(JSON.stringify(loaiSanPham));
	$.ajax({
		type : "POST",
		url : "add?",
...
function f_edit(id) {
	var loaiSanPham = new Object();
	loaiSanPham.id = id;
	loaiSanPham.tenLoai = $("#tenLoaiSanPhamChinhSua").val();
	var ngayCapNhat = new Date();
	loaiSanPham.ngayCapNhat = ngayCapNhat;
	loaiSanPham.tinhTrang = $("#tinhTrangChinhSua").val();
